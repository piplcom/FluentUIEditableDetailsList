...
const EditableGrid = (props: Props) => {
  ...
  const onDropDownChange = (
    ...
    activateCellEdit.forEach((item, index) => {
      if (row == index) {
        if (column.dropdownMultiSelect) {
          if (!item.properties[column.key].value) {
            item.properties[column.key].value = [];
          }

          if (selectedDropdownItem?.selected) {
            item.properties[column.key].value.push(selectedDropdownItem?.text);
          } else {
            const index = item.properties[column.key].value.indexOf(
              selectedDropdownItem?.text
            );
            if (index > -1) {
              item.properties[column.key].value.splice(index, 1);
            }
          }
                    ...
                    <span className={"row-" + rowNum! + "-col-" + index}>
                      {ShouldRenderSpan() ? (
                        ...
                        <Dropdown
                          ...
                          multiSelect={column.dropdownMultiSelect}
